...
export default function AdminDashboardHome() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/employees/stats`, {
        headers: {
      ...
      console.log('📊 Admin Dashboard: Received stats data:', data);
      setStats(data);
    } catch (error) {
  ...
  const fetchRecentActivities = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin/employees?limit=5`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      const data = await response.json();
      console.log('📊 Admin Dashboard: Received employees data:', data);
